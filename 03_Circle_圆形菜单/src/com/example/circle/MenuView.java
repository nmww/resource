...
public class MenuView extends ViewGroup {
	...
	public MenuView(Context context) {
		super(context);
		SMALL_RADIUS = (int) context.getResources()
				.getDimension(R.dimen.radius);
		init(context);
	}
	...
	public void out() {
		...
		for (int i = 0; i < length; i++) {
			...
			TranslateAnimation animation = new TranslateAnimation(startX
					- v.getLeft(), 0, startY - v.getTop(), 0);
	...
	public void in() {
		...
		for (int i = 0; i < length; i++) {
			final View v = views.get(i);
			TranslateAnimation animation = new TranslateAnimation(0, startX
					- v.getLeft() - v.getWidth() / 2, 0, startY - v.getTop()
					+ v.getHeight() / 2);
			animation.setInterpolator(new OvershootInterpolator(1.2f));
			DURATION += 500;
			animation.setDuration(DURATION);
			/*
			 * if (i >= length / 2) { DURATION += 500;
			 * animation.setDuration(DURATION); } else {
			 * animation.setDuration(DURATION); }
			 */
			v.startAnimation(animation);
			animation.setAnimationListener(new AnimationListener() {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (child != null) {
				child.measure(MeasureSpec.makeMeasureSpec(SMALL_RADIUS,
						MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(
						SMALL_RADIUS, MeasureSpec.EXACTLY));
			}
		}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (i == 0) {
				...
				int child_x = (int) (width / 2 - radiusBig
						* Math.cos(Math.PI / 6))
						+ PADDING_L_R;
				int child_y = (int) (height - radiusSmall - radiusBig
						* Math.sin(Math.PI / 6));

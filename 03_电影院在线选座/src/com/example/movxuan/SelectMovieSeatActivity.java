...
import com.example.gesturedetectors.MoveGestureDetector;
import com.example.model.SeatMo;

/**
 * @author captain_miao
 */
public class SelectMovieSeatActivity extends Activity implements
		OnTouchListener {
	...
	private MoveGestureDetector mMoveDetector;// 移动探测器
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		screenWidth = resources.getDisplayMetrics().widthPixels;// widthPixels：在像素的显示屏的绝对宽度
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		mFocusX = minLeft > 0 ? Math.max(-minLeft,
				Math.min(mFocusX, defWidth * mScaleFactor)) : Math.max(0,
				Math.min(mFocusX, defWidth * mScaleFactor));

		minTop = (int) (defWidth * mScaleFactor * maxRow)
				- seatTableView.getMeasuredHeight();
	...
	public void onChanged() {
		...
		for (int i = 0; i < seatTableView.getRowSize(); i++) {
			...
			textView.setPadding(
					...
					0, getResources()
							.getDimensionPixelSize(R.dimen.padding_2dp), 0);
	...
	private class ScaleListener extends
		...
		public boolean onScale(ScaleGestureDetector detector) {
			mScaleFactor *= detector.getScaleFactor(); // scale change since previous event 好像是判断手指是否是缩放或是放大
	...
	private void initSeatTable() {
		seatTable = new SeatMo[maxRow][maxColumn];// 模拟数据
